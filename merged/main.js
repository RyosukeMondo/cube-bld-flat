...
function render(result){
  ...
  for(const d of defs){
    ...
    rect.setAttribute('width',  wCells*U - 2);
    rect.setAttribute('height', hCells*U - 2);
    rect.setAttribute('rx', 4);
    rect.setAttribute('fill', d.fill || '#0f1421');
    ...
    rect.addEventListener('mousemove', (ev)=>{
      ...
      tip.style.opacity = 1;
      tip.textContent = `(${d.x1},${d.y1})-(${d.x2},${d.y2})  key:${d.key ?? '-'}  char:${d.char ?? ''}`;
    });
  ...
  for(const d of defs){
    ...
    t.setAttribute('y', y*U + (hCells*U)/2);
    t.setAttribute('dominant-baseline','middle');

    // アラインメント: 左帯は左寄せ、右帯は右寄せ、その他は中央
    const pad = 8;
    if(d.fill === COLORS.leftBand){
      t.setAttribute('text-anchor','start');
      t.setAttribute('x', x*U + pad);
    }else if(d.fill === COLORS.rightBand){
      t.setAttribute('text-anchor','end');
      t.setAttribute('x', (d.x2 + 1)*U - pad);
    }else{
      t.setAttribute('text-anchor','middle');
      t.setAttribute('x', x*U + (wCells*U)/2);
    }
    svg.appendChild(t);
  }
}
